...
- first local session lifecycle endpoints now exist: `GET /arena/sessions`, `POST /arena/sessions`, `GET /arena/sessions/:sessionID`, `POST /arena/sessions/:sessionID/agents`, `GET /arena/sessions/:sessionID/agents/:clientID/observation`, and `POST /arena/sessions/:sessionID/agents/:clientID/actions`. Joined agents without pending tickets receive `no_pending_action`. A minimal internal in-memory pending ticket model covers the smoke happy path `get observation -> submit matching turnID -> accepted`, an internal bridge can create those tickets from `AgentObservation`, accepted submitted actions can be retrieved once by future runner wiring, and expired tickets reject late matching submissions; live pull-style runner advancement, gameplay action application, and MCP action tools remain future slices.
