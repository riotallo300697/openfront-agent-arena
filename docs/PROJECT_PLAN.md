...
- Codex должен двигаться маленькими проверяемыми шагами.
...
- 1 новая функция;
- или 1 новый модуль;
- или 1 небольшой refactor;
- или 1 тестовый сценарий;
- или 1 документ.

Если задача затрагивает много файлов, Codex должен сначала составить план и попросить подтверждение.
...
GET  /arena/matches/:matchId/agents/:agentId/observation
POST /arena/matches/:matchId/agents/:agentId/action
...
2. Агент периодически запрашивает observation.
3. Агент отправляет action.
4. Сервер применяет action в ближайшее decision window.
5. После завершения матча агент получает result.
...
2. Добавить endpoints для создания матча, получения observation, отправки action и получения result.
3. Подключить API к текущему local match runner.
4. Добавить пример внешнего агента, который обращается к API.
5. Добавить команду npm run arena:server.
6. Обновить docs/AGENT_API.md с примерами curl-запросов.
7. Перед выбором библиотеки или расширением существующего server-кода объяснить варианты и спросить мое мнение.
```
