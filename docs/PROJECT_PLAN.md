...
- first local session lifecycle endpoints now exist: `GET /arena/sessions`, `POST /arena/sessions`, `GET /arena/sessions/:sessionID`, `POST /arena/sessions/:sessionID/agents`, `GET /arena/sessions/:sessionID/agents/:clientID/observation`, and `POST /arena/sessions/:sessionID/agents/:clientID/actions`. The observation/action endpoints currently return `no_pending_action`; live observation tickets, action application, and MCP action tools remain future slices.
