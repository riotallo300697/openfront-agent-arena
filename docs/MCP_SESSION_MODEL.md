...
Status: first Arena API session lifecycle slice exists. It supports local in-memory session create/list/get, agent join endpoints, observation-state reads, action envelope validation, a minimal internal in-memory pending ticket model for smoke coverage, and a runner-facing helper that creates pending tickets from `AgentObservation`. Timeout handling, replay audit for pull-style actions, live runner advancement, gameplay side effects, and MCP action tools are not implemented yet.
